#!/usr/bin/env python3
"""
Simplified update_services.py for Anthropic using unitysvc_services data builders.
...
class ModelExtractor:
    ...
    def get_code_examples(self, model_id: str) -> list[tuple[str, str, str]]:
        return [
            ...
            ("JavaScript code example", "../../docs/code-example.js.j2",
             "javascript"),
            ("cURL code example", "../../docs/code-example.sh.j2", "bash"),
            ("Python function calling code example",
             "../../docs/code-example-fc.py.j2", "python"),
        ]

    def get_documents(self) -> list[tuple[str, str, str, str]]:
        """Return additional documents like description."""
        return [
            ("How to use this model", "../../docs/description.md", "markdown",
             "getting_started"),
        ]
    ...
    def build_offering(self, model_id: str,
                       model_info: dict) -> OfferingDataBuilder:
        service_type = self.determine_service_type(model_id)
        display_name = model_id.replace("-", " ").replace("_", " ").title()

        builder = (OfferingDataBuilder(model_id).set_description(
            f"{display_name} language model").set_display_name(
                display_name).set_service_type(service_type).set_status(
                    "ready").add_tag("byop"))
        ...
        if litellm_details:
            ...
            if "max_input_tokens" in litellm_details:
                builder.add_detail("contextLength",
                                   litellm_details["max_input_tokens"])
            if "litellm_provider" in litellm_details:
                builder.add_detail("litellm_provider",
                                   litellm_details["litellm_provider"])
            if "input_cost_per_token" in litellm_details and "output_cost_per_token" in litellm_details:
                input_price = float(
                    litellm_details["input_cost_per_token"]) * 1_000_000
                output_price = float(
                    litellm_details["output_cost_per_token"]) * 1_000_000
    ...
    def build_listing(self, model_id: str) -> ListingDataBuilder:
        builder = (ListingDataBuilder().set_status("ready").add_user_interface(
            "Provider API",
            base_url=f"${{GATEWAY_BASE_URL}}/p/{PROVIDER_NAME}"))
        ...
        # Add additional documents (like description.md)
        for title, path, mime_type, category in self.get_documents():
            builder.add_document(title,
                                 path,
                                 mime_type=mime_type,
                                 category=category,
                                 description="")
        ...
        builder.set_raw("service_options", {
            "default_parameters": {
                "api_key": "${ secrets.ANTHROPIC_API_KEY }"
            }
        })
    ...
    def process_model(self, model, output_dir: Path) -> bool:
        ...
        # write() automatically skips if only time_created differs
        ...
        print(
            f"  {'Written: ' + ', '.join(status) if status else 'Unchanged'}")
        return True
